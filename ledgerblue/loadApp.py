...
parser.add_argument("--signApp", help="Sign application with provided signKey", action='store_true')
parser.add_argument("--signKey", help="Application signing key")
parser.add_argument("--appFlags", help="Set the application flags", type=auto_int)
parser.add_argument("--bootAddr", help="Set the boot offset address in the loaded file", type=auto_int)
...
if args.signApp and args.signKey == None:
	raise Exception("Can't sign app without the --signKey")
...
if (signature == None and args.signApp):
	masterPrivate = PrivateKey(bytes(bytearray.fromhex(args.signKey)))
	signature = masterPrivate.ecdsa_serialize(masterPrivate.ecdsa_sign(bytes(binascii.unhexlify(hash)), raw=True))
	print("Application signature: {}".format(binascii.hexlify(signature)))
